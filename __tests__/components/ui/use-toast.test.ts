...
describe('useToast Hook (UI)', () => {
  ...
  describe('Toast Auto-removal Timeout', () => {
    it('creates toast with auto-removal setup', () => {
      const { result } = renderHook(() => useToast())
      ...
      // The toast should have an onOpenChange handler for auto-dismissal
      expect(typeof result.current.toasts[0].onOpenChange).toBe('function')
    })
  })
})

describe('Toast Reducer (UI)', () => {
